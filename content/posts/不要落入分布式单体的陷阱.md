...
draft = false
title = '不要落入分布式单体的陷阱'
slug = 'avoid-the-distributed-monolith-trap'
aliases = ['/posts/不要落入分布式单体的陷阱/']
description = '剖析分布式单体（Distributed Monolith）的成因、特征与危害，结合架构对比图，给出避免微服务退化为分布式单体的实践建议。'
categories = ['架构设计']
tags = ['微服务', '分布式系统', '架构', '单体架构']
+++
